...
(function(global, factory) {
    ...
    function count$1(values, valueof) {
        ...
        if (valueof === undefined) {
            for (let value of values) {
                if (value != null && (value = +value) >= value) {++count;
                }
            }
        } else {
            let index = -1;
            for (let value of values) {
                if ((value = valueof(value, ++index, values)) != null && (value = +value) >= value) {++count;
                }
            }
        }
        return count;
    ...
    function bin() {
        ...
        function histogram(data) {
            ...
            var m = tz.length;
            while (tz[0] <= x0) tz.shift(),
                --m;
            while (tz[m - 1] > x1) tz.pop(),
                --m;
    ...
    function rank(values, valueof = ascending$3) {
        ...
        if (valueof.length !== 2) V = V.map(valueof),
            valueof = ascending$3;
    ...
    function transpose(matrix) {
        ...
        for (var i = -1,
                 m = min$2(matrix, length$2), transpose = new Array(m); ++i < m;) {
            for (var j = -1,
                     n, row = transpose[i] = new Array(n); ++j < n;) {
                row[j] = matrix[j][i];
            }
        }
        return transpose;
    ...
    function axis(orient, scale) {
        ...
        function axis(context) {
            ...
            if (context !== selection) {
                ...
                tickExit = tickExit.transition(context).attr("opacity", epsilon$6).attr("transform",
                    function(d) {
                        return isFinite(d = position(d)) ? transform(d + offset) : this.getAttribute("transform");
                    });

                tickEnter.attr("opacity", epsilon$6).attr("transform",
                    function(d) {
            ...
            tick.attr("opacity", 1).attr("transform",
                function(d) {
        ...
        axis.ticks = function() {
            return tickArguments = Array.from(arguments),
                axis;
        };
    ...
    function selection_select(select) {
        ...
        for (var groups = this._groups,
                 m = groups.length,
                 subgroups = new Array(m), j = 0; j < m; ++j) {
    ...
    function bindKey(parent, group, enter, update, exit, data, key) {
        var i, node, nodeByKeyValue = new Map,
    ...
    function selection_sort(compare) {
        ...
        for (var groups = this._groups,
                 m = groups.length,
                 sortgroups = new Array(m), j = 0; j < m; ++j) {
    ...
    function decompose(a, b, c, d, e, f) {
        ...
        if (scaleX = Math.sqrt(a * a + b * b)) a /= scaleX,
            b /= scaleX;
        if (skewX = a * c + b * d) c -= a * skewX,
            d -= b * skewX;
        if (scaleY = Math.sqrt(c * c + d * d)) c /= scaleY,
            d /= scaleY,
            skewX /= scaleY;
        if (a * d < b * c) a = -a,
            b = -b,
    ...
    function piecewise(interpolate, values) {
        if (values === undefined) values = interpolate,
    ...
    function poke() {
        var now = clock.now(),
            delay = now - clockLast;
        if (delay > pokeDelay) clockSkew -= delay,
            clockLast = now;
    }
    ...
    function sleep(time) {
        ...
        if (delay > 24) {
            ...
            if (!interval$1) clockLast = clock.now(),
    ...
    function interval(callback, delay, time) {
        var t = new Timer,
            total = delay;
        if (delay == null) return t.restart(callback, delay, time),
            t;
    ...
    function tweenValue(transition, name, value) {
        ...
        transition.each(function() {
            var schedule = set(this, id); (schedule.value || (schedule.value = {}))[name] = value.apply(this, arguments);
        });
    ...
    function brush$1(dim) {
        ...
        function brush(group) {
            ...
            var handle = group.selectAll(".handle").data(dim.handles,
                function(d) {
                    return d.type;
                });
            ...
            handle.enter().append("rect").attr("class",
                function(d) {
        ...
        function redraw() {
            ...
            if (selection) {
                ...
                group.selectAll(".handle").style("display", null).attr("x",
                    function(d) {
        ...
        Emitter.prototype = {
            beforestart: function() {
                if (++this.active === 1) this.state.emitter = this,
                    this.starting = true;
                return this;
            },
            ...
            brush: function(event, mode) {
                ...
                if (--this.active === 0) delete this.state.emitter,
        ...
        function started(event) {
            ...
            function moved(event) {
                ...
                for (const point of points) if (point.cur) point[0] = point.cur[0],
                    point[1] = point.cur[1];
                moving = true;
                noevent$1(event);
                move(event);
            ...
            function move(event) {
                ...
                switch (mode) {
                    ...
                    {
                        if (points[1]) {
                            if (signX) w1 = max$2(W, min$1(E, points[0][0])),
                                e1 = max$2(W, min$1(E, points[1][0])),
                                signX = 1;
                            if (signY) n1 = max$2(N, min$1(S, points[0][1])),
                                s1 = max$2(N, min$1(S, points[1][1])),
                                signY = 1;
                        } else {
                            if (signX < 0) dx = max$2(W - w0, min$1(E - w0, dx)),
                                w1 = w0 + dx,
                                e1 = e0;
                            else if (signX > 0) dx = max$2(W - e0, min$1(E - e0, dx)),
                                w1 = w0,
                                e1 = e0 + dx;
                            if (signY < 0) dy = max$2(N - n0, min$1(S - n0, dy)),
                                n1 = n0 + dy,
                                s1 = s0;
                            else if (signY > 0) dy = max$2(N - s0, min$1(S - s0, dy)),
                                n1 = n0,
                                s1 = s0 + dy;
                        }
                        break;
                ...
                if (lockX) w1 = selection[0][0],
                    e1 = selection[1][0];
                if (lockY) n1 = selection[0][1],
                    s1 = selection[1][1];
            ...
            function keydowned(event) {
                switch (event.keyCode) {
                    ...
                    { // ALT
                        if (mode === MODE_HANDLE) {
                            if (signX) e0 = e1 - dx * signX,
                                w0 = w1 + dx * signX;
                            if (signY) s0 = s1 - dy * signY,
                                n0 = n1 + dy * signY;
                            mode = MODE_CENTER;
                            move(event);
                        }
    ...
    function ribbon(headRadius) {
        ...
        function ribbon() {
            var buffer, s = source.apply(this, arguments),
    ...
    function density() {
        ...
        density.cellSize = function(_) {
            ...
            return k = Math.floor(Math.log(_) / Math.LN2),
                resize();
        };
        ...
        density.bandwidth = function(_) {
            ...
            return r = Math.round((Math.sqrt(4 * _ * _ + 1) - 1) / 2),
                resize();
        };
    ...
    class Voronoi {
            ...
            for (let j = 0; j < n; j += 2) {
                ...
                if (c0 === 0 && c1 === 0) {
                    ...
                    let S,
                        ...
                        sy1;
                    if (c0 === 0) {
                        if ((S = this._clipSegment(x0, y0, x1, y1, c0, c1)) === null) continue; [sx0, sy0, sx1, sy1] = S;
                    } else {
                        if ((S = this._clipSegment(x1, y1, x0, y0, c1, c0)) === null) continue; [sx1, sy1, sx0, sy0] = S;
                        e0 = e1,
        ...
        _clipInfinite(i, points, vx0, vy0, vxn, vyn) {
            ...
            if (P = this._clipFinite(i, P)) {
                for (let j = 0, n = P.length, c0, c1 = this._edgecode(P[n - 2], P[n - 1]); j < n; j += 2) {
                    ...
                    if (c0 && c1) j = this._edge(i, c0, c1, P, j),
                        n = P.length;
                }
            } else if (this.contains(i, (this.xmin + this.xmax) / 2, (this.ymin + this.ymax) / 2)) {
            ...
            if (P.length > 4) {
                for (let i = 0; i < P.length; i += 2) {
                    const j = (i + 2) % P.length,
                        k = (i + 4) % P.length;
                    if (P[i] === P[j] && P[j] === P[k] || P[i + 1] === P[j + 1] && P[j + 1] === P[k + 1]) P.splice(j, 2),
                        i -= 2;
                }
            }
            return j;
        ...
        _project(x0, y0, vx, vy) {
            ...
            if (vy < 0) { // top
                ...
                if ((c = (this.ymin - y0) / vy) < t) y = this.ymin,
                    x = x0 + (t = c) * vx;
            } else if (vy > 0) { // bottom
                if (y0 >= this.ymax) return null;
                if ((c = (this.ymax - y0) / vy) < t) y = this.ymax,
                    x = x0 + (t = c) * vx;
            }
            if (vx > 0) { // right
                if (x0 >= this.xmax) return null;
                if ((c = (this.xmax - x0) / vx) < t) x = this.xmax,
                    y = y0 + (t = c) * vy;
            } else if (vx < 0) { // left
                if (x0 <= this.xmin) return null;
                if ((c = (this.xmin - x0) / vx) < t) x = this.xmin,
                    y = y0 + (t = c) * vy;
            }
            return [x, y];
        }
    ...
    class Delaunay {
        ...
        renderPoints(context, r) {
            if (r === undefined && (!context || typeof context.moveTo !== "function")) r = context,
                context = null;
            r = r == undefined ? 2 : +r;
            const buffer = context == null ? context = new Path: undefined;
            const {
    ...
    function * flatIterable(points, fx, fy, that) {
        ...
        for (const p of points) {
            ...
            yield fy.call(that, p, i, points); ++i;
        }
    }
    ...
    function dsvFormat(delimiter) {
        ...
        function parse(text, f) {
            var convert, columns, rows = parseRows(text,
                function(row, i) {
    ...
    function dsvParse(parse) {
        return function(input, init, row) {
            if (arguments.length === 2 && typeof init === "function") row = init,
                init = undefined;
    ...
    function dsv(delimiter, input, init, row) {
        if (arguments.length === 3 && typeof init === "function") row = init,
            init = undefined;
    ...
    function add(tree, x, y, d) {
        ...
        while (node.length) {
            ...
            if (parent = node, !(node = node[i = bottom << 1 | right])) return parent[i] = leaf,
                tree;
        }
        ...
        if (x === xp && y === yp) return leaf.next = node,
    ...
    function tree_remove(d) {
        ...
        if (node.length) while (true) {
            ...
            if (parent[(i + 1) & 3] || parent[(i + 2) & 3] || parent[(i + 3) & 3]) retainer = parent,
                j = i;
        }
        ...
        if (!parent) return this._root = next,
            this;
    ...
    treeProto.copy = function() {
        ...
        if (!node.length) return copy._root = leaf_copy(node),
            copy;
                ...
                if (data) {
                    if (data.index > node.index) {
                        var x = xi - data.x - data.vx,
                            ...
                            l = x * x + y * y;
                        if (l < r * r) {
                            if (x === 0) x = jiggle(random),
                                l += x * x;
                            if (y === 0) y = jiggle(random),
                                l += y * y;
    ...
    function link$2(links) {
        var id = index$3,
            ...
            strengths, distance = constant$4(30),
            distances,
            nodes,
            count,
            bias,
    ...
    function manyBody() {
        var nodes, node, random, alpha, strength = constant$4( - 30),
            strengths,
            distanceMin2 = 1,
        ...
        function force(_) {
            ...
            for (alpha = _, i = 0; i < n; ++i) node = nodes[i],
                tree.visit(apply);
        }
    ...
    function radial$1(radius, x, y) {
        var nodes, strength = constant$4(0.1),
            strengths,
            radiuses;
    ...
    function exponent(x) {
        return x = formatDecimalParts(Math.abs(x)),
            x ? x[1] : NaN;
    }
    ...
    function formatLocale$1(locale) {
        ...
        function newFormat(specifier) {
            ...
            if (zero || (fill === "0" && align === "=")) zero = true,
                fill = "0",
                align = "=";
            ...
            function format(value) {
                ...
                if (comma && zero) value = group(padding + value, padding.length ? width - valueSuffix.length: Infinity),
                    padding = "";
    ...
    var boundsStream$2 = {
        ...
        polygonEnd: function() {
            ...
            if (areaRingSum$1 < 0) lambda0$1 = -(lambda1 = 180),
                phi0 = -(phi1 = 90);
    ...
    function bounds(feature) {
        ...
        if (n = ranges.length) {
            ...
            for (deltaMax = -Infinity, n = merged.length - 1, i = 0, a = merged[n]; i <= n; a = b, ++i) {
                b = merged[i];
                if ((delta = angle(a[1], b[0])) > deltaMax) deltaMax = delta,
                    lambda0$1 = b[0],
                    lambda1 = a[1];
            }
        }
    ...
    function centroidPointCartesian(x, y, z) {++W0;
        X0$1 += (x - X0$1) / W0;
        Y0$1 += (y - Y0$1) / W0;
        Z0$1 += (z - Z0$1) / W0;
    }
    ...
    function forwardRotationLambda(deltaLambda) {
        return function(lambda, phi) {
            return lambda += deltaLambda,
                [lambda > pi$1 ? lambda - tau$1: lambda < -pi$1 ? lambda + tau$1: lambda, phi];
        };
    }
        ...
        return function(sink) {
            ...
            var clip = {
                ...
                polygonEnd: function() {
                    ...
                    if (segments.length) {
                        if (!polygonStarted) sink.polygonStart(),
                            polygonStarted = true;
    ...
    function clipLine(a, b, x0, y0, x1, y1) {
        ...
        if (t0 > 0) a[0] = ax + t0 * dx,
            a[1] = ay + t0 * dy;
        if (t1 < 1) b[0] = ax + t1 * dx,
            b[1] = ay + t1 * dy;
        return true;
    }
    ...
    function ringRadians(ring) {
        return ring = ring.map(pointRadians),
            ring.pop(),
            ring;
    }
    ...
    PathString.prototype = {
        ...
        pointRadius: function(_) {
            if ((_ = +_) !== this._radius) this._radius = _,
                this._circle = null;
            return this;
        },
        polygonStart: function() {
    ...
    function enclose(circles) {
        ...
        while (i < n) {
            ...
            else e = encloseBasis(B = extendBasis(B, p)),
                i = 0;
        }
    ...
    function bimap(domain, range, interpolate) {
        var d0 = domain[0],
            ...
            r0 = range[0],
            r1 = range[1];
        if (d1 < d0) d0 = normalize(d1, d0),
            r0 = interpolate(r1, r0);
        else d0 = normalize(d0, d1),
    ...
    function transformer$2() {
        var domain = unit,
            range = unit,
            interpolate = interpolate$2,
            transform, untransform, unknown, clamp = identity$3,
    ...
    function transformer$1() {
        var x0 = 0,
            x1 = 1,
            t0, t1, k10, transform, interpolator = identity$3,
            clamp = false,
            unknown;
    ...
    function transformer() {
        var x0 = 0,
            ...
            x2 = 1,
            s = 1,
            t0, t1, t2, k10, k21, interpolator = identity$3,
        ...
        function arc() {
            ...
            // Or is it a circular or annular sector?
                ...
                if (rp > epsilon) {
                    ...
                    if ((da0 -= p0 * 2) > epsilon) p0 *= (cw ? 1 : -1),
                        a00 += p0,
                        a10 -= p0;
                    else da0 = 0,
                        a00 = a10 = (a0 + a1) / 2;
                    if ((da1 -= p1 * 2) > epsilon) p1 *= (cw ? 1 : -1),
                        a01 += p1,
                        a11 -= p1;
                    else da1 = 0,
                        a01 = a11 = (a0 + a1) / 2;
    ...
    function line(x, y$1) {
        ...
        function line(data) {
            var i, n = (data = array(data)).length,
                d,
                defined0 = false,
                buffer;
    ...
    function area(x0, y0, y1) {
        ...
        function area(data) {
            var i, j, k, n = (data = array(data)).length,
                d,
                defined0 = false,
                buffer,
                x0z = new Array(n),
    ...
    function link(curve) {
        ...
        function link() {
            ...
            output.lineStart();
            argv[0] = s,
                output.point( + x.apply(this, argv), +y$1.apply(this, argv));
            argv[0] = t,
                output.point( + x.apply(this, argv), +y$1.apply(this, argv));
            output.lineEnd();
            if (buffer) return output = null,
    ...
    Step.prototype = {
        ...
        lineEnd: function() {
            ...
            if (this._line || (this._line !== 0 && this._point === 1)) this._context.closePath();
            if (this._line >= 0) this._t = 1 - this._t,
                this._line = 1 - this._line;
        },
        point: function(x, y) {
            x = +x,
    ...
    function none$1(series, order) {
        ...
        for (var i = 1,
                 j, s0, s1 = series[order[0]], n, m = s1.length; i < n; ++i) {
            s0 = s1,
                s1 = series[order[i]];
            for (j = 0; j < m; ++j) {
    ...
    function diverging(series, order) {
        if (! ((n = series.length) > 0)) return;
        for (var i, j = 0,
                 d, dy, yp, yn, n, m = series[order[0]].length; j < m; ++j) {
    ...
    function silhouette(series, order) {
        if (! ((n = series.length) > 0)) return;
        for (var j = 0,
                 s0 = series[order[0]], n, m = s0.length; j < m; ++j) {
            for (var i = 0,
    ...
    function peak(series) {
        var i = -1,
            ...
            n = series.length,
            vi, vj = -Infinity;
        while (++i < n) if ((vi = +series[i][1]) > vj) vj = vi,
            j = i;
        return j;
    }
    ...
    function zoom() {
        ...
        function touchstarted(event, ...args) {
            ...
            for (i = 0; i < n; ++i) {
                ...
                else if (!g.touch1 && g.touch0[2] !== p[2]) g.touch1 = p,
                    g.taps = 0;
            }
        ...
        function touchended(event, ...args) {
            ...
            if (g.touch1 && !g.touch0) g.touch0 = g.touch1,
                delete g.touch1;
