 ...
 * @date 2013-04-09
 * 
 * 注意：此控件针对离散时间做了特殊处理，源代码中涉及到离散时间的都有注释。 <br/> 此次修改暂不影响原有控件的使用，如在使用过程中发现bug请及时提出。
 * 
 */
Ext.define('Ext.ux.GroupTabPanel', {
	...
	initComponent	: function(config) {
		...
		me.addEvents(
				/**
				 * @event beforetabchange Fires before a tab change (activated by {@link #setActiveTab}). Return false
				 *        in any listener to cancel the tabchange
				...
				/**
				 * @event tabchange Fires when a new tab has been activated (activated by {@link #setActiveTab}).
				 * @param {Ext.ux.GroupTabPanel}
				 *            grouptabPanel The GroupTabPanel
				 * @param {Ext.Component}
				...
				/**
				 * @event beforegroupchange Fires before a group change (activated by {@link #setActiveGroup}). Return
				 *        false in any listener to cancel the groupchange
				 * @param {Ext.ux.GroupTabPanel}
				 *            grouptabPanel The GroupTabPanel
				 * @param {Ext.Component}
				 *            newGroup The root group card that is about to be activated
				 * @param {Ext.Component}
				 *            oldGroup The root group card that is currently active
				 */
				'beforegroupchange',

				/**
				 * @event groupchange Fires when a new group has been activated (activated by {@link #setActiveGroup}).
				 * @param {Ext.ux.GroupTabPanel}
				 *            grouptabPanel The GroupTabPanel
				 * @param {Ext.Component}
	...
	onNodeSelect	: function(selModel, node) {
		...
		if (node.parentNode && node.parentNode.parentNode === null) {
			parent = node;
			// 对离散时间做特殊处理，目前是以名称作为判断依据，判断值与component.library.js中的createDiscretePanel配置的名称相同
			// 这里对离散时间做了特殊处理，对其他正常使用不造成影响，但是请注意其他正常使用时不要将title配置为判断条件中的名称
			if (node.data.text == '周期离散时间' || node.data.text == '任意离散时间' || node.data.text == '循环离散时间') {
				if (me.setActiveGroup(parent.get('id')) === false
						|| me.setActiveTab(node.getChildAt(0).get('id')) === false) {
					return false;
				}
			} else {
		...
		if (node.parentNode && node.parentNode.parentNode === null) {
			if (node.data.text == '周期离散时间' || node.data.text == '任意离散时间' || node.data.text == '循环离散时间') {
				parent.getChildAt(0).set('activeTab', true);
			} else {
	...
	setActiveTab	: function(cmp) {
		...
		if (me.fireEvent('beforetabchange', me, newTab, oldTab) !== false) {
			me.activeTab = newTab;
			// 注释掉源代码，去掉判断，针对离散时间特殊处理，暂时未发现对已有grouppanel造成影响，如果有影响请提出。
			// if (me.rendered) {
			// me.down('container[baseCls=' + Ext.baseCSSPrefix + 'grouptabcontainer' + ']').getLayout()
	...
	createTreeStore	: function() {
		...
		Ext.each(groups, function(groupItem, idx) {
			...
			if (groupRoot.activeTab) {
				// 对离散时间做特殊处理，目前是以名称作为判断依据
				// 判断值与component.library.js中的createDiscretePanel配置的名称相同
				// 这里对离散时间做了特殊处理，对其他正常使用不造成影响，但是请注意使用时不要将title配置为判断条件中的名称
				if (groupRoot.text == '周期离散时间') {
					groupRoot.activeTab = false;
					me.activeTab = leafItems[1].id;
				} else {
			...
			Ext.each(leafItems, function(leafItem) {
				...
				if (leafItem.id !== groupRoot.id) {
					...
					// 判断值与component.library.js中的createDiscretePanel配置的名称相同
					// 这里对离散时间做了特殊处理，对其他正常使用不造成影响，但是请注意使用时不要将title配置为判断条件中的名称
					if (groupRoot.text == '周期离散时间' && leafItem.title == '小时') {
						var child = {
							id			: leafItem.id,
							leaf		: true,
							text		: leafItem.title,
